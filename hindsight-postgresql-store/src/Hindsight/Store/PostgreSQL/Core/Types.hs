...
data SQLStoreHandle = SQLStoreHandle
  { pool :: !Pool,                    -- ^ Database connection pool
    connectionString :: !ByteString,  -- ^ Connection string (needed for LISTEN/NOTIFY)
    syncProjectionRegistry :: !SyncProjectionRegistry,  -- ^ Sync projections to execute with insertions
    notifier :: !Notifier  -- ^ Centralized notifier for subscriptions
  }
...
data SQLCursor = SQLCursor
  { transactionNo :: !Int64,  -- ^ Transaction number (globally increasing)
    sequenceNo :: !Int32      -- ^ Sequence within transaction (1, 2, 3...)
  }
...
data ExtendedSQLCursor = ExtendedSQLCursor
  { globalCursor :: !SQLCursor,       -- ^ Position in global event log
    streamVersion :: !StreamVersion   -- ^ Version within the specific stream
  }
  deriving stock (Show, Eq, Ord, Generic)
  deriving anyclass (FromJSON, ToJSON)