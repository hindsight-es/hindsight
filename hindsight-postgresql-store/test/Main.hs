...
-- SQL store test runner without timeout (for long-running stress tests)
sqlStoreRunnerNoTimeout :: EventStoreTestRunner SQLStore
sqlStoreRunnerNoTimeout =
    EventStoreTestRunner
        { withStore = \action ->
            void $
                let config =
...
sqlStoreTests = do
    return $
        testGroup
            ...
            , testGroup "Stress Tests" (stressTests sqlStoreRunner)
            , propertyTests sqlStoreRunner
            , testGroup "Ordering Tests" (orderingTests sqlStoreRunnerNoTimeout)
            , testGroup
