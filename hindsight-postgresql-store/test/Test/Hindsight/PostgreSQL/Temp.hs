...
import Hindsight.Store.PostgreSQL (SQLStoreHandle, SyncProjectionRegistry, createSQLSchema, getPool, newSQLStore, newSQLStoreWithProjections, shutdownSQLStore)

-- | Configuration for timeout behavior
data TimeoutConfig
    = -- | No timeout, test runs indefinitely
      NoTimeout
...
data DebugConfig
    = -- | No debugging output
      NoDebug
    | -- | Debug only on exceptions
      DebugOnFailure
    | -- | Debug only on timeouts
      DebugOnTimeout
    | -- | Debug on any failure (exceptions or timeouts)
      DebugOnAny
    deriving (Eq, Show)

-- | Configuration for schema initialization
data SchemaConfig
    = -- | Just create basic schema
      BasicSchema
    | -- | Schema + sync projections
      WithProjections SyncProjectionRegistry
...
data TempPostgresConfig = TempPostgresConfig
    ...
    , debugPauseDuration :: Int
    -- ^ How long to pause for debugging (microseconds)
    }
