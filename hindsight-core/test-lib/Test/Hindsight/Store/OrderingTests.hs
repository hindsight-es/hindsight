...
instance forall position. (Show position) => Show (TestSubscriptionResult position) where
    show r =
        "TestSubscriptionResult{"
            ++ "subId="
            ++ unpack r.resultSubscriptionId
            ++ ", events="
            ++ show r.resultEventCount
            ++ ", hash="
            ++ show r.resultFinalHash
            ++ ", completed="
            ++ show r.resultCompletionReceived
            ++ ", positions="
            ++ show (length r.resultPositionsSeen)
            ++ " items"
            ++ "}"
...
formatProgress s =
    let elapsed = diffUTCTime s.lastUpdateTime s.testStartTime
        ...
        etaStr =
            if txProgress > 0 && s.transactionsCompleted < s.totalTransactions
                then
                    let rate = fromIntegral txProgress / realToFrac elapsed :: Double
        ...
        subStatus =
            show s.subscriptionsCompleted
                ++ "/"
                ++ show s.totalSubscriptions
                ++ " ("
                ++ show subPercent
                ++ "%)"
                ++ " ["
                ++ show s.subscriptionsStarted
                ++ " started]"
     in "["
            ++ show elapsedSecs
            ++ "s] "
            ++ unpack s.currentPhase
            ++ " | Tx: "
            ++ txStatus
            ++ " | Subs: "
            ++ subStatus
...
updateHashChainGeneric prevHash position streamId payload streamHashes textHashes =
    let
     ...
     in
        (newHash, newStreamHashes, newTextHashes)
  where
...
testComprehensiveConsistencyWithConfig runner config = do
    ...
    putStrLn $
        "Config: "
            ++ show config.configNumTransactions
            ++ " transactions over "
            ++ show config.configTxExecutionWindowMs
            ++ "ms, "
            ++ show (config.configNumEarlySubscriptions + config.configNumConcurrentSubscriptions + config.configNumValidationSubscriptions)
            ++ " subscriptions over "
            ++ show config.configSubExecutionWindowMs
            ++ "ms"
...
analyzeResults results = do
    ...
    let (earlyResults, concurrentResults, validationResults) =
            foldr
                ( \r (e, c, v) ->
                    if "early-" `isPrefixOf` r.resultSubscriptionId
                        then (r : e, c, v)
                        else
                            if "concurrent-" `isPrefixOf` r.resultSubscriptionId
                                then (e, r : c, v)
                                else (e, c, r : v)
                )
                ([], [], [])
    ...
    putStrLn $
        "Completion status - Completed: "
            ++ show (length completedSubs)
            ++ ", Incomplete: "
            ++ show (length incompleteSubs)
    ...
    if uniqueHashes == 1
        then do
            ...
            when (minEvents /= maxEvents) $
                putStrLn $
                    "⚠️  Event count mismatch: min=" ++ show minEvents ++ ", max=" ++ show maxEvents
